/*
 * Pulled from http://www.vetra.com/scancodes.html
 * Via:
 *   console.log(
 *     JSON.stringify(
 *       [...document.querySelectorAll('tbody')[2].querySelectorAll('tr')].map(v => {
 *         return [...v.querySelectorAll('td')].map(v => v.innerText)
 *       })
 *     )
 *   );
 */
...
export const scancodes = [
    [" IBM Key No.", "Set 1 Make/Break", "Set 2 Make/Break", "Set 3 Make/Break", "Base\nCase", " Upper Case"],
    ...
    .map(([ibmKeyNo, set1MakeBreak, set2MakeBreak, set3MakeBreak, baseCase, upperCase]) => {
        function parseMakeBreak(makeBreak: string) {
            const [a, b] = makeBreak.split('/');
            return {
                make: a.split(' ').map(v => parseInt(v, 16)),
                break: b.split(' ').map(v => parseInt(v, 16)),
            }
        }
        const set1 = parseMakeBreak(set1MakeBreak);
        const set2 = parseMakeBreak(set2MakeBreak);
        const set3 = parseMakeBreak(set3MakeBreak);
        return {
            ibmKeyNo,
            set1,
            set2,
